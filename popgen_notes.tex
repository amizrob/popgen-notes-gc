...
\subsection{Diploid selection model}
...
Another potential reason for the persistence of deleterious alleles in a
...
This seems unlikely to be as broad an explanation as mutation--selection balance for the
persistence of deleterious alleles on a genome-wide scale. However, a brief discussion of such alleles is worthwhile, as it helps to inform our ideas about local adaptation.\\

As a first pass at this, let us consider a haploid two-allele model with
two different populations, where the relative fitnesses of the two alleles
$A_1$ and $A_2$ are as follows:
\begin{center}
	\begin{tabular}{c|cc}
		allele & $A_1$ & $A_2$ \\
		\hline
...
Hence, allele $A_1$ is advantageous in population 1, but deleterious in population 2, and vice versa for allele $A_2$. Let us denote the frequency of $A_1$ and $A_2$ in population 1 by $p_1$ and $q_1$, respectively. Analogously, we use $p_2$ and $q_2$ for the frequency of $A_1$ and $A_2$ in population 2. As a simple model of migration, we assume that migration between the two populations is symmetric in the sense that, each generation, a fraction of $m$ individuals are migrants from  the other population, and a fraction of $1-m$ are residents from the population itself.\\

In each population, selection favours the respective locally beneficial allele. At the same time, migration introduces the locally deleterious allele at rate $m$. This sounds very similar to the scenario of the previous section, where selection was acting against deleterious alleles that are constantly introduced via mutation. Indeed, to study the interaction of selection and migration, we will set up a situation analogous
to the one we encountered above when studying mutation--selection balance. Let us assume that selection is strong compared to migration ($s \gg m$), so that allele
$A_1$ will be almost fixed in population $1$ ($p_1 \approx 1$, $q_1 \approx 0$) and allele $A_2$ will be
almost fixed in population $2$ ($q_2 \approx 1$, $p_1 \approx 0$). If that is the case,
migration changes the frequency $q_1$ of allele $A_2$ in population $1$ by
\begin{equation}
	\Delta_{\mathrm{Mig}} q_1 = m q_2 + (1-m) q_1 - q_1 = m (q_2 - q_1) \approx m.
\end{equation}
Note the similarity to eqn.\ \eqref{eq:mutApprox}. As we have seen above, the change due to selection is $\Delta_{S} q_1= -sq_1$, so that migration and
selection are at an equilibrium when $\Delta_{S} q_1+
\Delta_{\mathrm{Mig}}q_1 = 0$. This implies an equilibrium frequency of allele $A_2$ in
population $1$ of
\begin{equation}
	q_{e,1} = \frac{m}{s}.
\end{equation}
Comparing to eqn.\ \eqref{eqn:mut_sel_bal}, migration is playing to role of mutation here; 
migration--selection balance is
analogous to mutation selection--balance, at least if selection is strong relative to migration and mutation.\\

\subsubsection{The spatial distribution of allele
frequencies under deterministic models of selection}
Imagine a continuous haploid population spread out along a line. 
Individuals disperse a random distance $\Delta x$ from their birthplace to the
location where where they reproduce, where $\Delta x$ is drawn from the probability
density $g(~)$. For simplicity, we will assume that $g(\Delta x)$
is a normal distribution with mean zero and standard deviation
$\sigma$, i.e.\ migration is unbiased in terms of direction, and individuals migrate an average
distance of $\sigma$. \\

The frequency of allele $A_2$ at time $t$ in the population at spatial
location $x$ is $q(x,t)$. Assuming that only dispersal occurs, how
does $q(x,t)$ change in the next generation? The allele
frequency in the next generation at location $x$ must reflect
 migration of individuals from different locations in the current
generation to $x$ in the next generation. The local `population' at $x$ receives a contribution
$g(\Delta x)q(x+\Delta x,t)$ of allele $A_2$ from the population at location $x+\Delta x$,
such that the frequency of allele $A_2$ at $x$ in the next generation is
\begin{equation}
	q(x,t+1) = \int_{-\infty}^{\infty} g(\Delta x)q(x+\Delta x,t) d \Delta x.
\end{equation}

To obtain $q(x+\Delta x,t)$, let us take a Taylor series expansion of $q(x,t)$ as follows:
\begin{equation}
	q(x+\Delta x,t) = q(x,t) + \Delta x \frac{dq(x,t)}{dx}+ \tfrac{1}{2}(\Delta x)^2 \frac{d^2q(x,t)}{dx^2}+\mathcal{O}(\Delta x)^3.
\end{equation}
Then, ignoring terms of order $\mathcal{O}(\Delta x)^3$ and higher,
\begin{equation}
	q(x,t+1) \approx q(x,t) +\left( \int_{-\infty}^{\infty} \Delta x g(\Delta x) 
  d \Delta x \right) \frac{dq(x,t)}{dx} + \tfrac{1}{2}\left( \int_{-\infty}^{\infty}(\Delta x)^2 g(\Delta x) 
  d \Delta x \right)  \frac{d^2q(x,t)}{dx^2}.
\end{equation} 
Recall that $g(~)$ has a mean of zero, so that $ \int_{-\infty}^{\infty} \Delta x g(\Delta x) 
  d \Delta x =0$, and $g(~)$ has variance $\sigma^2$, so that $\int_{-\infty}^{\infty}(\Delta x)^2 g(\Delta x) 
  d \Delta x = \sigma^2$. Last, as  $g(~)$ is a normal distribution, all higher terms are zero. Therefore, the change in frequency, $\Delta q(x,t) = q(x,t+1)-q(x,t)$, is simply given by
\begin{equation}
	\Delta q(x,t) = \frac{\sigma^2}{2} \frac{d^2q(x,t)}{dx^2}.
\end{equation} 
This is a diffusion equation. It tells us that migration is acting to smooth
out allele frequency differences across space with a diffusion constant
of $\sigma^2/2$. This is exactly analogous to the equation
describing how a gas diffuses out to equal density. Both particles
in a gas and individuals of type $A_2$ are performing Brownian
motion (if we blur our eyes and take time as continuous). \\


We will now add selection to our model and set the
relative fitnesses of allele $A_1$ and $A_2$ at location $x$ to be $1$ and
$1+s\gamma(x)$, respectively. To make progress in this model, we will have to assume that selection
is not too strong, i.e.\ $s \gamma(x) \ll 1$ for all $x$. The change in
frequency of allele $A_2$ within a generation due to selection is
\begin{equation}
	q^{\prime}(x,t) - q(x,t) \approx s\gamma(x) q(x,t) \big( 1 - q(x,t) \big).
\end{equation}
Hence, at location $x$, the favoured allele grows logistically. We have seen similar approximations before for the haploid and diploid model of selection.

Putting together the terms for selection and migration, we find 
\begin{equation}
	q(x,t+1) - q(x,t) = s\gamma(x) q(x,t) \big( 1 - q(x,t).
\big)+\frac{\sigma^2}{2} \frac{d^2q(x,t)}{dx^2} \label{eqn:fisherKPP}
\end{equation}
In derving this, we have essentially assumed that migration acted upon the original
frequencies before selection, and in doing so, we have ignored terms of the
order of $\sigma s$. 
...
\end{center}
\caption{An equilibrium cline in allele frequency. Individuals
  disperse an average distance of $\sigma=1$ km per generation, and 
allele $A_2$ has a relative fitness of $1+s$ and $1-s$ on either side of
the the boundary ($x = 0$) at which the environmen changes.}
\label{fig:cline}
\end{figure}


To gain some more insight, let us consider a simple model of location adaptation
where the environment abruptly changes. Specifically, we assume that
$\gamma(x)= -1$ for $x<0$ and $\gamma(x)= 1$ for $x \geq 0$. Hence, allele $A_2$ has a
...
get an equilibrium distribution of the two alleles such that to the left
of $x = 0$ allele $A_2$ is at higher frequency, while to the right of
$x=0$ allele $A_1$ predominates. As we cross from the left to the right
side of the boundary, the frequency of allele $A_2$ decreases in a
smooth cline. Note that the transition is gradual, even though the change in the environment is abrubt. This is because migration smooths out some of the differences across the sharp boundary. \\
...
conditions ($q(-\infty)=1$ and $q(\infty) = 0$) to obtain the
appropriate functional form to our cline. While we will not go into the
solution of this equation here, we can note that by dividing the
distance $x$ by $\ell=\sigma/\sqrt{s}$, we can remove the effect of the
parameters from the above equation. This compound parameter $\ell$ is the characteristic
length of the cline. It is this parameter that determines over
what geographic scale we change from allele $A_2$ predominating to
allele $A_1$ predominating as we move across the environmental shift. \\

The width of the cline, i.e.\ over what distance we make this shift
from allele $A_2$ predominating to allele $A_1$,
...
stochastic. A selection coefficient of 1 \% ($s = 0.01$) is a strong
selection coefficient and, in a deterministic model, can drive an allele through the
...
offspring per generation, even though the first individual to carry the beneficial allele has, on average, $1 \%$ more offspring than the rest, it could easily happen to have zero offspring. This would lead to the loss of the allele before it ever has a chance to spread.\\
